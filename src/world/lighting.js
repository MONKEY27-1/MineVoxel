...
/** @param blocksBySection Uint8Array(4096)[16]  @param skyLightBySection Uint8Array(4096)[16] */
export function computeSkyLight(blocksBySection, skyLightBySection) {
  ...
  for (let x = 0; x < SIZE; x++) {
    for (let z = 0; z < SIZE; z++) {
      for (let y = HEIGHT - 1; y >= 0; y--) {
        if (isOpaque(readBlock(blocksBySection, x, y, z))) break;
  ...
  spread(blocksBySection, skyLightBySection, queue, /* verticalDownFalloff */ 0);
}
...
export function computeBlockLight(blocksBySection, blockLightBySection) {
  ...
  spread(blocksBySection, blockLightBySection, queue, /* verticalDownFalloff */ 1);
}
...
function spread(blocksBySection, lightBySection, queue, verticalDownFalloff) {
  let qi = 0;
  while (qi < queue.length) {
    const x = queue[qi++];
    const y = queue[qi++];
    const z = queue[qi++];
    const light = readLight(lightBySection, x, y, z);
    if (light <= 1) continue;
    ...
    for (const [dx, dy, dz] of DIRS) {
      ...
      if (isOpaque(readBlock(blocksBySection, nx, ny, nz))) continue;

      const falloff = dy === -1 ? verticalDownFalloff : 1;
      const propagated = light - falloff;
      if (propagated > readLight(lightBySection, nx, ny, nz)) {
...
export function recomputeColumnLight(column) {
  ...
  computeSkyLight(blocksBySection, skyArrays);
...
export function __selfTestLighting() {
  ...
  const under = readLight(skyLightBySection, 8, 10, 8); // straight down the shaft
  const besideUnderRoof = readLight(skyLightBySection, 9, 19, 8); // one step sideways, under the roof
  const farUnderRoof = readLight(skyLightBySection, 2, 19, 2); // far from the hole, under the roof
  const belowFloor = readLight(skyLightBySection, 8, -1, 8);
