...
## Post-launch fix: terrain generation didn't actually resemble the Nether

Reported by the user: "make the generation like the nether." Investigating turned up a real, previously-unverified bug: `cinderdeepGenerator.js`'s original cave-noise threshold (0.62 near the shell margin, 0.3 deep) had a comment claiming "measured at this threshold the volume reads as maybe a third solid, two-thirds open" — but nobody had actually measured it. Sampling the live game directly (via the debug hook, reading real generated block data) found **only ~6% open space** — the Cinderdeep was almost entirely solid Cinderstone, nothing like a Nether you can walk or fly through. Every uniform-terrain biome read as generic dark caves regardless, with no shape distinction from real Nether biomes (Soul Sand Valley, Crimson/Warped Forest, Basalt Deltas all have a completely different silhouette from Nether Wastes in the real game).

Fixed in `cinderdeepGenerator.js`/`cinderdeepBiomes.js`, in two parts:

1. **Recalibrated the cave threshold** by sampling the real noise fields directly instead of guessing — landed on `0.24`/`-0.08` (from `0.62`/`0.3`), consistently measuring 35-65% open across every seed sampled. This alone turns Cinder Wastes from "solid rock with rare pockets" into an actual connected cavern network.
2. **Gave 3 biome groups their own terrain *shape***, not just their own wall/floor material, via a new `isOpen(wx, wy, wz, biome)` dispatcher keyed on biome config flags (never a biome-id branch, matching the "config, not a check" rule everywhere else in this dimension):
   - **Mourning Flats / Bloodcap Grove / Azurecap Hollow** (Soul Sand Valley / Crimson Forest / Warped Forest equivalents) get a new `flatValley: true` flag and `isOpenValley` — a relatively flat, walkable floor-to-ceiling band (rolling floor ~26-58, ~22-36 blocks of headroom) instead of a pocketed 3D cave network. These read as open realms you cross, matching the real biomes' silhouette, not caverns you spelunk through.
   - **Basalt Fractures** (Basalt Deltas) gets `isOpenDelta` — a bumpy solid terrain *surface* (open above it, solid below), not a cave network at all. Wherever the surface dips below the dimension's lava-sea level, the gap fills with lava the same way the rest of the lava sea does, giving natural lava lakes in the delta's own low ground. The existing full-height pillar override (`isPillarColumn`) frequency was also raised substantially (from ~14% to ~30% of columns) so pillars/spires actually dominate the biome's look, matching real basalt deltas, instead of being a rare accent on top of what used to be a generic open cavern.
